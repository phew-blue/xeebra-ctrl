...
export default function ShutdownModal({
  ...
  const stepLabel: Record<Step, string> = {
    confirm: 'Ready to shutdown',
    stopping: 'Stopping configuration...',
    ssh: 'Connecting via SSH...',
    success: 'Shutdown initiated successfully',
    ...
    >
      <div className="bg-evs-gray-dark border border-evs-gray rounded shadow-xl w-full max-w-md mx-4">
        ...
        <div className="flex items-center justify-between p-5 border-b border-evs-gray">
          <h2 className="font-medium text-evs-contrast">Shutdown Server</h2>
          {!processing && (
        ...
        <div className="p-5 space-y-4">
          ...
          {step === 'confirm' && (
            <div className="p-3 bg-evs-warning/10 border border-evs-warning/30 rounded text-sm text-evs-gray-lighter">
              This will stop the server configuration if running, then shut down the physical machine.
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-evs-gray">
          {step === 'confirm' && (
            <>
              ...
              <button
                ...
                className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white rounded text-sm"
              >
                Shutdown
              </button>
            </>
          ...
          {step === 'error' && (
            <>
              ...
              <button onClick={handleConfirm} className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white rounded text-sm">Retry</button>
            </>
          )}
          {step === 'success' && (
          ...
          {(step === 'stopping' || step === 'ssh') && (
            <span className="text-sm text-evs-gray-lighter py-2">Shutdown in progress...</span>
          )}
        </div>
      </div>
    </div>
